...
// A class used for modeling tetrominoes with 7 out of 7 different types/shapes
// as (I, O, Z, S, T, L and J)
public class Tetromino {
    ...
    public char getType() { // for next Tetromino
        return type;
    }
    ...
    public boolean move(String direction, GameGrid gameGrid) {
        ...
        if (direction == "left") // 1 unit of left move
            bottomLeftCell.setX(bottomLeftCell.getX() - 1);
        else if (direction == "right") // 1 unit of right move
            bottomLeftCell.setX(bottomLeftCell.getX() + 1);
        else if (direction == "down") // 1 unit of down move
            bottomLeftCell.setY(bottomLeftCell.getY() - 1);
        else { // direction == "fulldown" // move down as much as possible
    ...
    // A method to rotate right side
    public void rotateRight() {
        int n = tileMatrix.length; // to know how many columns and rows
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n / 2; j++) {
                Tile temp = tileMatrix[j][i];
                tileMatrix[j][i] = tileMatrix[n - j - 1][i];
                tileMatrix[n - j - 1][i] = temp;
            }
        }
        for (int i = 0; i < n; i++) {
            for (int j = i; j < n; j++) {
    ...
    // A method to rotate left side
    public void rotateLeft() {
        int n = tileMatrix.length; // to know how many columns and rows
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n / 2; j++) {
                Tile temp = tileMatrix[i][j];
                tileMatrix[i][j] = tileMatrix[i][n - j - 1];
                tileMatrix[i][n - j - 1] = temp;
            }
        }
        for (int i = 0; i < n; i++) {
            for (int j = i; j < n; j++) {
